...
export const Hero = () => {
  ...
  return (
    <div className="flex w-full min-h-screen flex-col justify-center items-center gap-10 bg-black">
      <div className="flex flex-col gap-4 items-center">
        <div className="p-4 bg-white/30 border border-white rounded-xl flex flex-col gap-4 items-center">
          ...
          <div className="flex items-center gap-4 ">
            ...
            <button
              onClick={async () => {
                await handleTransaction(name, setGreet, setList);
              }}
              className="bg-cyan-900 text-white p-3 rounded-md font-bold text-lg shadow hover:bg-cyan-400 active:bg-cyan-900 hover:text-black  ease-in-out duration-500 active:duration-0"
            >
              Send transaction
            </button>
          </div>
        ...
        <div className="flex flex-col gap-4 w-full">
          {list.map((item: any, index: number) => (
            <div key={index} className="p-3 border-b border-white w-full">
              Hello, {item}!
            </div>
          ))}
        </div>
